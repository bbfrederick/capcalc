...
def statestats(thestates, numlabels, minlabel, minout=1, minhold=1, debug=False):
    ...
    for i in range(numlabels):
        ...
        if len(lenarray) > 2:
            thestats.append(
                [100.0 * np.sum(lenarray) / len(thestates), len(lenarray), np.sum(lenarray), np.min(lenarray),
                 np.max(lenarray), np.mean(lenarray), np.median(lenarray), np.std(lenarray)])
        elif len(lenarray) > 1:
            thestats.append(
                [100.0 * np.sum(lenarray) / len(thestates), len(lenarray), np.sum(lenarray), np.min(lenarray),
                 np.max(lenarray), np.mean(lenarray), lenarray[1], 0.0])
        elif len(lenarray) > 0:
            thestats.append(
                [100.0 * np.sum(lenarray) / len(thestates), len(lenarray), np.sum(lenarray), lenarray[0], lenarray[0],
                 lenarray[0], lenarray[0], 0.0])
        else:
