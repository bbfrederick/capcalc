...
def main():
    ...
    template_img, template_data, template_hdr, thedims, thesizes = ccalc_io.readfromnifti(
        templatefile
    )
    ...
    for thecluster in range(numclusters):
        ...
        if isroifile:
            ...
            output_data[:, thecluster] += (
                theclustercenters[i - 1, thecluster] * templatevoxels[:, i]
            )
